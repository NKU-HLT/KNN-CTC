...
class ASRModel(torch.nn.Module):
    ...
    def knn_ctc_prefix_beam_search(
        self,
        speech: torch.Tensor,
        speech_lengths: torch.Tensor,
        beam_size: int,
        ...
        # here we insert knn
        softmax_outputs, dists, vals_at_knns = knn_wrapper.process(encoder_out, ctc_probs)
        knn_ctc_probs = softmax_outputs.squeeze(0)
        ...
        for t in range(0, maxlen):
            ...
            logp = knn_ctc_probs[t]  # (vocab_size,)
            # key: prefix, value (pb, pnb), default value(-inf, -inf)
            next_hyps = defaultdict(lambda: (-float('inf'), -float('inf')))
            # 2.1 First beam prune: select topk best
            top_k_logp, top_k_index = logp.topk(beam_size)  # (beam_size,)
    ...
    def ctc_prefix_beam_search(
        ...
        """ Apply knn-CTC prefix beam search
